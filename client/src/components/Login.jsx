    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 to-steel-100">
      <div className="w-full max-w-md">
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    ...
                    {error.includes('Invalid') && (
                      <div className="text-xs mt-1">
                        Please check your Active Directory credentials.
                      </div>
                    )}
                  </div>
                </div>
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-4 p-4 bg-steel-800 text-white rounded-lg text-xs font-mono">
            <div className="font-semibold mb-2">Debug Info:</div>
            <div>Origin: {window.location.origin}</div>
            <div>Cookies: {document.cookie || 'none'}</div>
            <div>API Base: {axios.defaults.baseURL || 'relative'}</div>
          </div>
        )}
      </div>
    </div>
