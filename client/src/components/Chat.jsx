...
function Chat({ user, handleLogout }) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // ✅ CRITICAL: Create assistant message with attachedFiles from response
      const assistantMsg = {
        role: 'assistant',
      ...
      // ✅ CRITICAL: Add attachedFiles if present
      if (response.data.attachedFiles && response.data.attachedFiles.length > 0) {
        assistantMsg.attachedFiles = response.data.attachedFiles;
        console.log('📎 Attached files:', assistantMsg.attachedFiles);
      }
